...
SDL_Surface *rotozoomSurfaceXY(SDL_Surface * src, double angle, double zoomx, double zoomy, int smooth)
{
	 ...
	 */
	rz_src =
	    SDL_CreateRGBSurface(SDL_SWSURFACE|SDL_SRCALPHA, src->w, src->h, 32, 0x000000ff, 0x0000ff00, 0x00ff0000, 0xff000000);
	...
	if (is32bit) {
	     ...
	     */
	    rz_dst =
		SDL_CreateRGBSurface(SDL_SWSURFACE|SDL_SRCALPHA, dstwidth, dstheight, 32,
				     rz_src->format->Rmask, rz_src->format->Gmask,
				     rz_src->format->Bmask, rz_src->format->Amask);
		SDL_FillRect(rz_dst, NULL, src->format->colorkey);
	} else {
	    /*
	     * Target surface is 8bit 
	     */
	    rz_dst = SDL_CreateRGBSurface(SDL_SWSURFACE|SDL_SRCALPHA, dstwidth, dstheight, 8, 0, 0, 0, 0);
